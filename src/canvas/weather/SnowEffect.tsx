...
function makeFlake(w: number, h: number, initial = false): Flake {
  return { x: Math.random() * w, y: initial ? Math.random() * h : -10, r: 1.2 + Math.random() * 2.4, vy: 0.5 + Math.random() * 1.2, swing: 0.4 + Math.random() * 1.2, phase: Math.random() * Math.PI * 2 };
}

function drawSnow(ctx: CanvasRenderingContext2D, width: number, height: number, flakes: Flake[], time: number): void {
