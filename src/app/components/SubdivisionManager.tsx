...
import { z } from 'zod'
import { SubdivisionConfigInputSchema, type SubdivisionConfig, type Texture, PIECE_CATEGORIES } from '@/pieces'
...
export function SubdivisionManager({ isOpen, onClose, subdivisions, allTextures }: Props) {
  ...
  const handleSubmit = (data: FormValues) => {
    ...
    startTransition(async () => {
      const result = editingId ? await updateSubdivision(editingId, data) : await createSubdivision(data)
      if (result.success) {
        onClose()
      } else {
        ...
        <div className="subdivision-manager">
                  ...
                  <li key={s.id} className={editingId === s.id ? 'editing' : ''}>
                    <div className="subdivision-row-info">
                      ...
                      <span className="subdivision-row-meta">
                        {s.textureIds.length} textura(s) · ratio: {s.cellSizeRatio} · order: {s.order}
                      </span>
                    </div>
                    <div className="subdivision-row-actions">
                      ...
                      <button type="button" className="button danger mini" onClick={() => handleDelete(s.id)}>
                        ×
                      </button>
                    </div>
                  </li>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="subdivision-manager-form">
            ...
            <div className="form-row">
              <FormField label="Ratio" required error={form.formState.errors.cellSizeRatio?.message} hint="1=64px, 2=32px, 4=16px, 8=8px">
                  ...
                  return (
                    <div className="texture-picker-list">
                      {visibleTextures.map((texture) => (
                        <label key={texture.id} className={`texture-picker-row ${selected.has(texture.id) ? 'selected' : ''}`}>
                          <input type="checkbox" checked={selected.has(texture.id)} onChange={() => toggle(texture.id)} />
